...
function ResetForm() {
  ...
  const formStyle = {
    ...
    btn: "w-[25.4rem] h-[4.5rem] items-center rounded-[1rem] shadow-[0px_0.4rem_1.9rem_0px_rgba(0,0,0,0.25)] w-[50%] poppins text-[1.4rem] font-medium leading-[normal] max-md:w-[23.4rem]",
    icons: "text-[1.5rem] hidden max-md:text-[1.2rem]",
  };
  const xml = (
    <form className={`${formStyle.form}`}>
      <h2 className={`${formStyle.header}`}>Reset Password</h2>
      {/* PASSWORD */}
      ...
      <div className={`${formStyle.btnHolder}`}>
        ...
        <Link
          className={`${formStyle.btn} text-center !leading-[4.5rem] bg-[#FFFFFF] text-[#61A061]`}
          to="/login"
        >
          Cancel
        </Link>
