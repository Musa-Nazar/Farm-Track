...
function Hero() {
  const xml = (
    <section className="w-full h-[65.8rem] hero flex max-md:h-[23rem]">
      <div className="flex flex-col gap-[5rem] mr-auto mt-[9.9rem] items-start max-md:mt-[4rem] max-md:gap-[1rem] max-md:ml-[1.6rem] w-[128rem] max-w-[85%] mx-auto">
        <h1 className="text-[#fff] text-[7.2rem] leading-0 font-[500] max-md:text-white  max-md:text-[25px] max-md:leading-[32.388px]">
          <span
            className="typing leading-[10.3rem]"
            style={{ "--delay": "0s" }}
          >
        ...
        <p
          className="text-[#fff] poppins text-[2.1rem] font-[400] tracking-[0.042rem] leading-[200%] max-md:text-[1rem] max-md:tracking-[0.02rem] typing"
          style={{ "--delay": "1s" }}
        >
          Track your stock, sales, and expenses in one place. <br />
          Get real-time alerts and reports.
        </p>
        <div className="flex gap-[5.6rem] max-md:gap-[calc(5.6rem/2)] overflow-hidden">
          <Link
            to={"/signup"}
